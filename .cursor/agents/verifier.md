...
description: >
  ...
  verification gate (`pnpm run ci` - lint, format, check-types, types-check, boundaries, test, build) plus
...
- You **NEVER** edit source or tests. If a test reveals a real defect, report the exact command
  ...
  make it green (see `.cursor/rules/quality/testing.mdc`).
- Do NOT run `pnpm types` (`wrangler types`) - it regenerates a generated `.d.ts`; leave that
  to the human or `pnpm types` (generated files are outputs, not sources).
...
or silently read as, "tests passed" - that is the failure `guardrails.mdc` prohibits under
