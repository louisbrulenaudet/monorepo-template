...
- You **NEVER** edit source or tests. If a test reveals a real defect, report the exact command plus the failing assertion and stop - do not weaken source, loosen a type, or skip a case to make it green (see `.cursor/rules/quality/testing.mdc`).
...
The `TESTS:` line is **mandatory on every run**. "No suite exists" must never be reported as, or silently read as, "tests passed" - that is the failure `guardrails.mdc` prohibits under "Do not paper over failures".
