...
`worker-configuration.d.ts` is generated by `wrangler types` and **committed** (per Cloudflare's recommendation), so a fresh clone type-checks with no generate step. It provides the `Env` interface (bindings/vars) and runtime API types matched to your `compatibility_date`. Re-run `make types` after editing `wrangler.jsonc` and commit the result; `make ci` verifies freshness via `make types-check`.
