...
- Reach for `make lint-agent` (or `pnpm lint:agent`), never bare `oxlint`. It pins `--format=agent`: one line per diagnostic, `file:line:col: severity plugin(rule): message help: …`. Act on those lines directly - location and fix hint are already there, so do not re-read the file to locate the problem. Read-only, no auto-fix.
- Never `cd` into a package to lint. Lint/format are whole-repo by design - see **Scoping** in `AGENTS.md` for why, and pass a path to narrow: `pnpm exec oxlint apps/front-app`.
...
- Run `make ci` before finishing any code change. Advisory - nothing enforces it. Delegate it to `verifier` when you do not need the raw diagnostics in context.
