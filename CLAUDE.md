...
- Run `pnpm run ci` before finishing any code change - one `turbo run` with `--continue=dependencies-successful`, so a single run surfaces every failure rather than only the first. Advisory - nothing enforces it. Delegate it to `verifier` when you do not need the raw diagnostics in context. While iterating, `pnpm run check` (seconds) and `pnpm run ci:affected` are the cheaper tiers; `pnpm run ci:agent` is the machine-readable variant.
