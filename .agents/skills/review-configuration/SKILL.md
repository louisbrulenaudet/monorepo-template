...
description: >
  Config, env, wrangler review. USE WHEN: user runs /review-configuration or explicitly asks for this review.
...
- Use `@file` for config files (wrangler.jsonc, vite.config.ts, tsconfig.json, .oxfmtrc.json, .oxlintrc.json).
- Use `@code` for env or config snippets when suggesting changes.
- Use `@docs` for Cloudflare/OXC/TypeScript when checking correct options.
...
2. **Improvements** – Worthwhile (documentation of env, clearer TS/OXC rules, compatibility date update).
