...
live in [guardrails.md](../../../.claude/rules/core/guardrails.md) → "Privileged client data",
...
- **Secrets.** Only `.dev.vars` locally and wrangler secrets or env bindings in deployed
  environments. Never in `wrangler.jsonc` `vars`, never in a log, never in an error body.
