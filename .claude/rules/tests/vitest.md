...
- Meaningful assertions on observable behavior (status, body, headers, binding effects) - reject `toBeDefined()`-only. Prefer real pool bindings on Workers; mock only true I/O on Node, through `vi.fn` / `vi.stubGlobal` or an injected fake - never module mocking (`vi.mock` & co. are lint errors via `vitest/no-restricted-vi-methods`).
