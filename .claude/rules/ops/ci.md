...
- **Parity:** root `pnpm ci` and the workflow run the same *kinds* of checks. Change both when adding/renaming a gate. Intentional difference: CI uses `turbo … --affected` for typecheck/test/build; local `pnpm ci` is the full graph (also a single `turbo run check-types test build` invocation). Keep checkout **`fetch-depth: 0`** + **`filter: blob:none`**, and set **`TURBO_SCM_BASE`** to `origin/<base_ref>` on PRs (checkout only creates remote-tracking refs), so `--affected` works.
- **Parallelization:** one job; GitHub Actions `parallel:` for OXC (`lint`/`format`), `boundaries`, and `types:check`. A **single** `turbo run check-types test build --affected` (not two turbo CLIs); turbo schedules against default concurrency (no override in `turbo.json`). Wrangler deploy dry-run is `front-app` only after that join (`worker-api` build is already a dry-run).
- **Install:** `pnpm install --frozen-lockfile` after `pnpm/setup` with `install: false`. Node `runtime: node@24` matching root engines.
- **Remote cache:** job env wires `TURBO_TOKEN`, `TURBO_TEAM`, and `TURBO_REMOTE_CACHE_SIGNATURE_KEY` (repo secret, >= 32 bytes, same value as local machines - turbo.json enables `remoteCache.signature` + `longerSignatureKey`).
