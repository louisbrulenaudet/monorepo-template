...
- **Parallelization:** one job; GitHub Actions `parallel:` for OXC (`lint`/`format`), `boundaries`, and `types:check`. A **single** `turbo run check-types test build --affected` (not two turbo CLIs) so `global.concurrency: "100%"` is not oversubscribed. Wrangler deploy dry-run is `front-app` only after that join (`worker-api` build is already a dry-run).
