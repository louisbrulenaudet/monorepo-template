...
Styling for the React SPAs. Tailwind **v4**, CSS-first (no `tailwind.config.js`). Component/hook authoring is in [react.md](react.md); Vite plugin wiring in [vite-config.md](vite-config.md); cross-cutting layout in [frontend-architecture.md](frontend-architecture.md). Motion/interaction depth → **`ui-ux-design-best-practices`** skill.
...
- The build runs through `@tailwindcss/vite` (Lightning CSS is built in - no separate Autoprefixer/PostCSS step). Plugin order lives in [vite-config.md](vite-config.md): `tailwindcss()` after `react()`/`babel`, before `cloudflare()`.
...
## Theming & dark mode (semantic tokens)
...
## Writing classes in components

- Utilities in JSX; factor repetition into `src/components/ui/` primitives composed with `cx()` (see `Button.tsx` / `Card.tsx`).
- Variants are a `Record<Variant, string>` of **complete, static class strings**. Never build a class by interpolation (`` `bg-${color}-500` ``) - the scanner reads source as plain text and won't emit interpolated classes. Enforced by `no-concatenated-classes`.
- A **runtime** value goes in `style` as a CSS variable, consumed by an arbitrary-value utility using v4 **parentheses** shorthand: `className="bg-(--brand)"` (not v3 `bg-[--brand]`). `bg-(--brand)` == `bg-[var(--brand)]`.
- Prefer the shorthand utilities the linter enforces (`size-2.5`, not `h-2.5 w-2.5`). Motion/transition patterns → `ui-ux-design-best-practices`.
...
- **The plugin resolves `entryPoint` against the process CWD, not against `.oxlintrc.json`.** OXC therefore always runs from the repo root. `oxlint .` from inside `apps/front-app` resolves it to `apps/front-app/apps/front-app/src/index.css`, which makes the theme-aware rules emit a "No tailwind css entry point found" banner on every message and makes `no-unknown-classes` reject every `@theme` token. Scope by path instead: `pnpm exec oxlint apps/front-app`.
- Enabled (all `error`): `enforce-canonical-classes`, `no-concatenated-classes`, `no-conflicting-classes`, `no-deprecated-classes` (catches v3 names in v4), `no-duplicate-classes`, `no-unknown-classes`, `no-unnecessary-whitespace`. The four that need the theme: `enforce-canonical-classes`, `no-conflicting-classes`, `no-deprecated-classes`, `no-unknown-classes`.
- Class **ordering** is not linted - `oxfmt` owns it natively via `sortTailwindcss.stylesheet` (same algorithm as `prettier-plugin-tailwindcss`, and unlike the lint plugin it resolves the path relative to `.oxfmtrc.json`, so it is CWD-independent). `enforce-consistent-class-order` stays **off** so the two cannot disagree.
...
Run `make ci`, then `pnpm --filter front-app run build` to confirm CSS compiles and chunks resolve. Keep every Oxc rule green (see [code-style.md](../quality/code-style.md)); never silence a rule to pass (see [guardrails.md](../core/guardrails.md)).
