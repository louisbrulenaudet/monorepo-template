...
- **The plugin resolves `entryPoint` against the process CWD, not against `.oxlintrc.json`.** OXC therefore always runs from the repo root. `oxlint .` from inside `apps/front-app` resolves it to `apps/front-app/apps/front-app/src/index.css`, which makes the theme-aware rules emit a "No tailwind css entry point found" banner on every message and makes `no-unknown-classes` reject every `@theme` token. Scope by path instead: `pnpm --filter=front-app run lint`.
...
Run `pnpm run ci`, then `pnpm --filter front-app run build` to confirm CSS compiles and chunks resolve. Keep every Oxc rule green (see [code-style.md](../quality/code-style.md)); never silence a rule to pass (see [guardrails.md](../core/guardrails.md)).
