...
- Keep `queryFn` pure and typed: delegate the fetch to the service module's `fetchJsonWithSchema` helper (it forwards the `signal` for auto-cancel, throws a real `Error` on non-OK, and **validates the response** with the shared Zod schema - never `as T`). See [contracts.md](../contracts/contracts.md) / [type-inference.md](../contracts/type-inference.md).
...
export const todoDetailQuery = (id: number) =>
  queryOptions({
    ...
    queryFn: async ({ signal }) => {
      ...
      if (!res.ok) {
        throw new Error(`Todo ${id} failed: ${res.status}`);
      }
...
- Create **one** `QueryClient` as a module singleton and pass it to a single `<QueryClientProvider>` (wiring lives in [frontend-architecture.md](frontend-architecture.md)). Never `new QueryClient()` in a component body, and never run multiple clients (it splits the cache and defeats dedup).
- Set sensible `defaultOptions.queries` (e.g. `staleTime: 60_000`, `gcTime: 5 * 60_000`, `retry: 2`, `refetchOnWindowFocus` per product need). Never set `staleTime > gcTime`.
- Mount `<ReactQueryDevtools>` **dev-only** (`import.meta.env.DEV`) so it is tree-shaken from production.
