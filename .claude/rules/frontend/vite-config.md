...
`devtools` → `tanstackRouter` (**before** `react`) → `react()` → `babel({ presets: [reactCompilerPreset()] })` → `tailwindcss` → `cloudflare` → build-only (`apply: "build"`) → conditional via spread. Falsy plugins are skipped. Compiler on → less manual memoization ([react.md](react.md)).
...
- Leave `build.target` unset (Vite 8 default `baseline-widely-available`). `sourcemap`: inline (development mode) / `false` (production). `reportCompressedSize: false`; `cssCodeSplit: true`. Leave `modulePreload.polyfill` at its Vite default `true` unless you intentionally drop older-browser preload support.
