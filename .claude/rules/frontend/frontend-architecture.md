...
- Colocate feature code (route + its queries + its components) over deep global folders. Use package.json `"imports"` (`#/*` → `./src/*`) for in-app absolute imports (e.g. `#/components/ui/Button`); see [vite-config.md](vite-config.md) and the TypeScript config rule.
