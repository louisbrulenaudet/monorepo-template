...
- Colocate feature code (route + its queries + its components) over deep global folders. Reuse the existing path aliases — `@/*` plus the app-specific ones (`@utils`, `@enums`, `@components`, `@ui`, `@routes`, `@pages`, `@hooks`, `@services`, `@config`) — declared in `tsconfig.json` **and** `vite.config.ts`; keep the two in sync.
- Filenames stay kebab-case; a React component file may be PascalCase to mirror its export. See [naming.md](naming.md).

## Vite plugin order & the React Compiler

- Plugin order matters: `tanstackRouter({ target: "react", autoCodeSplitting: true })` must come **before** `@vitejs/plugin-react`. The working order in this repo (see `apps/front-app/vite.config.ts`) is: `devtools()` → `tanstackRouter(...)` → `react()` → `babel({ presets: [reactCompilerPreset()] })` → `tailwindcss()` → `cloudflare()` → generated-headers plugin (build-only) → optional `visualizer`.
- Prefer enabling the **React Compiler**. This repo pins `@vitejs/plugin-react` v6 (Vite 8), which **removed the `babel` option** from `react()` — so add the compiler as a **separate Babel plugin entry** via `@rolldown/plugin-babel` with `reactCompilerPreset()` (both exported from `@vitejs/plugin-react`), placed right after `react()` as this app does. With the compiler on, drop manual memoization by default (see [react.md](react.md)). If you don't enable it, memoize manually where it matters.
...
## Cloudflare Workers deployment & build

- These SPAs deploy as **static assets + SPA routing on Cloudflare Workers**. Wire this with `@cloudflare/vite-plugin` and configure the deploy in `wrangler.jsonc`. Styling uses `@tailwindcss/vite` (Tailwind v4). Router/query devtools are mounted through `@tanstack/devtools-vite` and stay dev-only (see below).
- Security headers (`Content-Security-Policy`, `Permissions-Policy`, etc.) are generated at **build time** into `dist/_headers` from `VITE_API_BASE_URL` by a small build-only Vite plugin — never hand-write `dist/_headers`. A build guard also fails the production build when `VITE_API_BASE_URL` is missing or a placeholder origin.
...
Run `make ci`, then build the app (`pnpm --filter front-app run build`, or `make build` from the repo root) to confirm the bundle compiles and chunks resolve. Keep every Oxc rule green (see [code-style.md](code-style.md)).
