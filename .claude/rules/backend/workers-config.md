...
`wrangler.jsonc` is the **source of truth** for a Worker's bindings, vars, and routes. Generated outputs must never be hand-edited (see [guardrails.md](../core/guardrails.md)):
...
| Environment | Deploy flag | Purpose |
|-------------|-------------|---------|
...
## Optional performance / DX options

- `limits.cpu_ms` - CPU guard (default 30000, max 300000); lower for thin workers, raise for heavy routes.
- `placement` - `smart` / `region` / `host` / `hostname`; only for workers fronting a centralized backend, never a pure edge gateway.
...
Missing required config must fail predictably, not degrade. Preserve application-level 503 guards (see [guardrails.md](../core/guardrails.md)).
...
Because service bindings are not package dependencies, the caller also declares explicit `worker-foo#transit` dependencies for `check-types`, `types`, and `types:check` in its package `turbo.json`. Keep the existing same-package `transit` dependency on `check-types`.
...
4. Caller: add explicit callee `#transit` edges in package `turbo.json`.
5. Shared schemas in `dtos-common/rpc` if needed.
6. `pnpm types` → `pnpm check-types`.

## Discipline

- After editing any `wrangler.jsonc`, run `pnpm types` and `pnpm check-types`.
