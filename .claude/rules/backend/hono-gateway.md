...
2. **`sentry(app, (env) => …)`** (`@sentry/hono/cloudflare`) - right after `requestId()`, followed by an inline middleware that `setTag("request_id", …)`s the opaque id. It reports `c.error` 5xx even when `onError` handles the error, so do not add `captureException` there. Keep `dataCollection` locked down (no user info, cookies, headers, bodies, query params, frame variables) - privileged client data, see [guardrails.md](../core/guardrails.md).
3. **`secureHeaders()`** (`hono/secure-headers`) - HSTS, `nosniff`, `no-referrer`, COOP/CORP on by default (and `X-Powered-By` removed). Add a locked-down `contentSecurityPolicy` (`default-src 'none'; frame-ancestors 'none'`) - harmless for JSON, hardening if a response is ever rendered. Set `permissionsPolicy` to deny camera/mic/geolocation/payment. CORP `same-origin` is fine for CORS-mode `fetch` from `front-app`; do not disable it to fix SPA reads.
4. **`cors()`** on `/api/*` - preflight `OPTIONS` must clear before auth/routes. Read the allowlist from `c.env.CORS_ORIGINS` **per request** (env is not available at module scope on Workers). Put `X-Request-Id` in `exposeHeaders` so browsers can read the id. Empty allowlist is permissive (`*`) in `dev` only; any other `ENVIRONMENT` (including `staging` / `production` and typos) must set origins or `/api/*` returns **503**.
5. **Origin / CSRF gate** on `/api/*` - unsafe methods only (any `Content-Type`, including `application/json`); must not block CORS preflight. Allows when Origin is on the allowlist **or** `Sec-Fetch-Site` is `same-origin` / `same-site`. Hono built-in `csrf()` only covers form content-types - do not rely on it alone for this JSON SPA gateway.

**API sub-app (`/api/v1`):**

6. **`timing()`** (`hono/timing`) - Server-Timing header, **dev only** (gate on `c.env.ENVIRONMENT`). Workers timer metrics are inaccurate (timers advance only on I/O) and internal timings should not leak to clients.
7. **`timeout(ms)`** (`hono/timeout`) - safety net returning `504`. Read the caveat below before adding routes.
8. **`bodyLimit()`** - e.g. 3 MB cap; return `413` on overflow.
9. **`prettyJSON()`** - dev only.
